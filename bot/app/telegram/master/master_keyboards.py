...
def get_master_main_menu(lang: str = "uk") -> InlineKeyboardMarkup:
    ...
    builder.button(
        text=t("master_schedule_button", lang), callback_data=pack_cb(MasterMenuCB, act="schedule")
    )
    builder.button(
        text=t("my_bookings_button", lang), callback_data=pack_cb(MasterMenuCB, act="bookings")
    )
    builder.button(
        text=t("master_my_clients_button", lang),
        callback_data=pack_cb(MasterMenuCB, act="my_clients"),
    )
    builder.button(
        text=t("master_stats_button", lang), callback_data=pack_cb(MasterMenuCB, act="stats")
    )
    # New: edit per-service durations
    builder.button(
        text=t("master_service_durations_button", lang),
        callback_data=pack_cb(MasterMenuCB, act="service_durations"),
    )
...
def get_weekly_schedule_kb(include_edit: bool = True, lang: str = "uk") -> InlineKeyboardMarkup:
    ...
    builder.button(
        text=t("master_refresh_button", lang),
        callback_data=pack_cb(MasterScheduleCB, action="refresh"),
    )
    builder.button(
        text=t("master_clear_all_button", lang),
        callback_data=pack_cb(MasterMenuCB, act="clear_all"),
    )
            ...
            if packed_time:
                ...
                builder.button(
                    ...
                    callback_data=pack_cb(
                        MasterScheduleCB, action="remove_window", day=day, time=packed_time
                    ),
                )
            else:
                ...
                builder.button(
                    text=f"🗑 {label}",
                    callback_data=pack_cb(MasterScheduleCB, action="remove_window", day=day),
                )
    else:
        builder.button(
            text=t("master_no_windows", lang),
            callback_data=pack_cb(MasterScheduleCB, action="noop", day=day),
        )

    # Add / actions row
    builder.button(
        text=t("master_add_window_button", lang),
        callback_data=pack_cb(MasterScheduleCB, action="add_time", day=day),
    )
...
def get_master_bookings_dashboard_kb(
    lang: str = "uk", mode: str = "upcoming", page: int = 1, total_pages: int = 1
) -> InlineKeyboardMarkup:
...
def get_time_start_kb(
    day: int, *, times: list[str] | None = None, lang: str = "uk"
) -> InlineKeyboardMarkup:
    ...
    for tstr in times:
        ...
        builder.button(
            text=tstr,
            callback_data=pack_cb(MasterScheduleCB, action="pick_start", day=day, time=token),
        )
    builder.adjust(4)
    ...
    try:
        back_button = InlineKeyboardButton(
            ...
            callback_data=pack_cb(MasterScheduleCB, action="edit_day", day=day),
        )
...
def get_time_end_kb(
    ...
    *,
    ...
    """
    logger.debug(
        "Building time end kb for day=%s start_time=%s end_hour=%s step=%s",
        day,
        start_time,
        end_hour,
        step_min,
