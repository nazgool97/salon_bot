...
async def _load_master_bookings(
    ...
    *,
    ...
    try:
        ...
        logger.exception(
            "_load_master_bookings: resolve_master_id failed for %s", master_telegram_id
        )
        resolved_mid = None
    if not resolved_mid:
        return [], default_meta
...
async def master_edit_note_fallback(msg: Message, state: FSMContext, locale: str) -> None:
    ...
    logger.info(
        "master_edit_note_fallback: entered state with text_len=%s from_user=%s",
        len(text),
        msg.from_user.id,
    )
    ...
    async def _restore_previous_screen() -> None:
        ...
        if user_id and master_id:
            try:
                hist = await master_services.MasterRepo.get_client_history_for_master_by_user(
                    int(master_id), int(user_id)
                )
                text_card = (
                    master_services.format_client_history(hist or {}, int(user_id), lang=lang)
                    if hist
                    else t("master_no_client_history", lang)
                )
                builder = InlineKeyboardBuilder()
                builder.button(
                    text=t("edit_note_button", lang),
                    callback_data=pack_cb(MasterClientNoteCB, action="edit", user_id=int(user_id)),
                )
                builder.button(
                    text=t("back", lang),
                    callback_data=pack_cb(callbacks_mod.MasterMenuCB, act="my_clients"),
                )
                builder.adjust(2)
    ...
    try:
        if booking_id:
            logger.info(
                "master_edit_note_fallback: attempting upsert_client_note booking_id=%s", booking_id
            )
            ok = await master_services.MasterRepo.upsert_client_note(booking_id, text)
            logger.info(
                "master_edit_note_fallback: upsert_client_note result=%s for booking_id=%s",
                ok,
                booking_id,
            )
        elif user_id and master_id:
            logger.info(
                "master_edit_note_fallback: attempting upsert_client_note_for_user master_id=%s user_id=%s",
                master_id,
                user_id,
            )
            ok = await master_services.MasterRepo.upsert_client_note_for_user(
                int(master_id), int(user_id), text
            )
            logger.info(
                "master_edit_note_fallback: upsert_client_note_for_user result=%s for master_id=%s user_id=%s",
                ok,
                master_id,
                user_id,
            )
...
async def master_my_clients(
    ...
    if page < 1:
        ...
        if not clients:
            try:
                await safe_edit(
                    cb.message,
                    text=t("master_no_clients", lang),
                    reply_markup=get_master_main_menu(lang),
                )
                await nav_push(
                    state, t("master_no_clients", lang), get_master_main_menu(lang), lang=lang
                )
            except Exception:
    ...
    if total_pages > 1:
        if page > 1:
            builder.button(
                text=t("page_prev", lang),
                callback_data=pack_cb(MasterMenuCB, act="my_clients", page=page - 1),
            )
        else:
...
async def master_client_note_edit(
    ...
    try:
        ...
        builder.button(
            text=t("cancel", lang),
            callback_data=pack_cb(MasterClientNoteCB, action="cancel_edit", user_id=int(user_id)),
        )
        kb = builder.as_markup()
...
async def show_schedule(cb: CallbackQuery, state: FSMContext, locale: str) -> None:
    ...
    logger.debug(
        "show_schedule invoked by user=%s data=%s", cb.from_user.id, getattr(cb, "data", None)
    )
    master_id = cb.from_user.id
    if master_id is None:
...
async def schedule_remove_window(
    ...
    try:
        ...
        success, conflicts = await master_services.remove_schedule_window_by_value(
            int(master_id), int(day), start_val, end_val
        )
        if not success:
            if conflicts:
                try:
                    ids = await master_services.check_future_booking_conflicts(
                        int(master_id), clear_all=True, horizon_days=365, return_ids=True
                    )
                except Exception:
                    ids = []
                count = len(ids or [])
...
async def _check_and_confirm_day_clear(
    ...
    try:
        ...
        ids = await master_services.check_future_booking_conflicts(
            int(master_id), day_to_clear=int(day), horizon_days=365, return_ids=True
        )
        if ids:
            count = len(ids)
            from aiogram.utils.keyboard import InlineKeyboardBuilder
        ...
        try:
            ...
            logger.exception(
                "_check_and_confirm_day_clear: show_schedule failed, falling back to inline render"
            )
...
async def schedule_add_time(
    ...
    try:
        ...
        await safe_edit(
            cb.message, text=tr("master_select_window_start", lang=lang), reply_markup=start_kb
        )
    except TelegramAPIError:
        logger.exception(
            "Telegram API error showing start-time kb for master %s day=%s", master_id, day
        )
    return
...
async def schedule_confirm_clear_day(
                ...
                try:
                    if send_booking_notification:
                        await send_booking_notification(
                            cast(Bot, bot), int(bid), "cancelled", recipients
                        )
                    else:
                        logger.warning(
                            "send_booking_notification helper unavailable, skipping notification for %s",
                            bid,
                        )
                except Exception:
    ...
    try:
        ...
        logger.exception(
            "Failed to mark day off after cancelling bookings for master %s day=%s", master_id, day
        )
        await safe_edit(cb.message, text=t("error_retry"))
        return
    try:
...
async def show_bookings_menu(cb: CallbackQuery, state: FSMContext, locale: str) -> None:
    ...
    try:
        ...
        kb = await build_my_bookings_keyboard(
            ...
            (
                meta.get("done_count", 0)
                + meta.get("cancelled_count", 0)
                + meta.get("noshow_count", 0)
            ),
            "upcoming",
            meta.get("page", 1),
            lang,
...
async def confirm_clear_all(cb: CallbackQuery, locale: str) -> None:
    ...
    builder = InlineKeyboardBuilder()
    builder.button(
        text=t("confirm", lang), callback_data=pack_cb(MasterMenuCB, act="clear_all_confirm")
    )
    ...
    builder.adjust(2)
    await safe_edit(
        cb.message, text=tr("master_clear_all_confirm"), reply_markup=builder.as_markup()
    )
            ...
            if ok:
                ...
                if bot and client_tid:
                    ...
                    try:
                        ...
                        logger.exception(
                            "Failed to notify recipients for cancelled booking %s", bid
                        )
        except Exception:
            continue
...
async def pick_window_start(
    ...
    try:
        ...
        await safe_edit(
            cb.message,
            text=tr("master_select_window_end", start=start_time, lang=lang),
            reply_markup=end_kb,
        )
    except Exception as e:
...
async def master_service_durations_menu(cb: CallbackQuery, state: FSMContext, locale: str) -> None:
    ...
    try:
        ...
        rows = await master_services.MasterRepo.get_services_with_durations_for_master(
            int(master_id)
        )
        from aiogram.utils.keyboard import InlineKeyboardBuilder
            ...
            for sid, name, dur in rows:
                ...
                kb.button(
                    ...
                    callback_data=pack_cb(
                        MasterSetServiceDurationCB, service_id=sid, minutes=dur or 0
                    ),
                )
    ...
    try:
        ...
        for m in options:
            ...
            kb.button(
                text=f"{prefix}{m}m",
                callback_data=pack_cb(
                    MasterSetServiceDurationCB, service_id=str(service_id), minutes=m
                ),
            )
        ...
        if current and int(current) > 0:
            kb.button(
                ...
                callback_data=pack_cb(
                    MasterMenuCB,
                    act="save_service_duration",
                    page=int(current) if isinstance(current, int) else None,
                ),
            )
        kb.button(
            text=t("back", lang), callback_data=pack_cb(MasterMenuCB, act="service_durations")
        )
        kb.adjust(4, 4, 2)
...
async def master_save_service_duration(cb: CallbackQuery, state: FSMContext, locale: str) -> None:
    ...
    try:
        ...
        for sid, name, dur in rows:
            ...
            kb.button(
                text=label,
                callback_data=pack_cb(MasterSetServiceDurationCB, service_id=sid, minutes=dur or 0),
            )
        kb.button(text=t("back", lang), callback_data=pack_cb(MasterMenuCB, act="menu"))
        kb.adjust(*([1] * max(1, len(rows))), 1)
        header = t("master_service_durations_header", lang)
...
@safe_handler()
async def master_bookings_navigate(
    cb: CallbackQuery, callback_data: _HasModePage, state: FSMContext, locale: str | None = None
) -> None:
        ...
        if ok:
            ...
            try:
                ...
                if recipients:
                    ...
                    logger.info(
                        "Notified client(s) about booking %s completion: %s", booking_id, recipients
                    )
            except Exception as ne:
                logger.exception(
                    "Failed to notify client about completion for booking %s: %s", booking_id, ne
                )
        else:
            await cb.answer(t("error_retry"))
    except TelegramAPIError:
    ...
    try:
        ...
        if bd:
            if isinstance(bd, dict):
                ...
                note = (
                    getattr(bd.raw, "get", lambda k, d=None: None)("client_note")
                    if getattr(bd, "raw", None)
                    else None
                )
        text = (
            t("client_note_label", lang)
            + ":\n\n"
...
async def booking_cancel_confirm(
    ...
    reason_kb.button(
        text=t("master_cancel_reason_ill", locale),
        callback_data=pack_cb(MasterCancelReasonCB, booking_id=booking_id, code="ill"),
    )
    reason_kb.button(
        text=t("master_cancel_reason_emergency", locale),
        callback_data=pack_cb(MasterCancelReasonCB, booking_id=booking_id, code="emergency"),
    )
    reason_kb.button(
        text=t("master_cancel_reason_error", locale),
        callback_data=pack_cb(MasterCancelReasonCB, booking_id=booking_id, code="error"),
    )
    reason_kb.button(
        text=t("master_cancel_reason_other", locale),
        callback_data=pack_cb(MasterCancelReasonCB, booking_id=booking_id, code="other"),
    )
    reason_kb.button(text="❌", callback_data=pack_cb(MasterMenuCB, act="my_clients"))
    reason_kb.adjust(2, 2, 1)
    await cb.answer()
    await safe_edit(
        cb.message, t("master_cancel_reason_prompt", locale), reply_markup=reason_kb.as_markup()
    )
    return
...
async def master_cancel_reason_text_input(
    msg: _MasterMsgType, state: FSMContext, locale: str
) -> None:
    lang = locale
...
async def booking_add_note(
    ...
    try:
        logger.info(
            "booking_add_note: invoked by master=%s booking_id=%s", cb.from_user.id, booking_id
        )
    ...
    try:
        ...
        if res:
            ...
            try:
                ...
                kb2.button(
                    text=t("booking_mark_done_button"),
                    callback_data=pack_cb(BookingActionCB, act="mark_done", booking_id=booking_id),
                )
                kb2.button(
                    text=t("booking_mark_noshow_button"),
                    callback_data=pack_cb(
                        BookingActionCB, act="mark_noshow", booking_id=booking_id
                    ),
                )
                kb2.button(
                    text=t("booking_client_history_button"),
                    callback_data=pack_cb(
                        BookingActionCB, act="client_history", booking_id=booking_id
                    ),
                )
                kb2.button(
                    text=t("booking_add_note_button"),
                    callback_data=pack_cb(BookingActionCB, act="add_note", booking_id=booking_id),
                )
                try:
                    kb2.button(
                        text=t("to_list", locale),
                        callback_data=pack_cb(MasterMenuCB, act="my_clients"),
                    )
                except Exception:
                    kb2.button(
                        text=t("menu", locale),
