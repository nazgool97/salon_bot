...
# order dependencies and duplicate handlers.


# Master edit-note FSM handling moved to `master_router`.
# Rely on router-level error handlers for global error handling.
...
async def start_booking(cb: CallbackQuery, callback_data, state: FSMContext, locale: str) -> None:
            ...
            ms_rows = (
                await session.execute(
                    select(MasterService.service_id, MasterService.duration_minutes).where(
                        MasterService.service_id.in_(service_ids)
                    )
                )
            ).all()
    ...
    for s in services:
        ...
        if ms_list:
            ...
            if mn == mx:
                decorated[sid_str] = (
                    f"{name} · {mn} {unit}" if unit != "minutes_short" else f"{name} · {mn}m"
                )
            else:
            ...
            if isinstance(prof_minutes, int) and prof_minutes > 0:
                decorated[sid_str] = (
                    f"{name} · {prof_minutes} {unit}"
                    if unit != "minutes_short"
                    else f"{name} · {prof_minutes}m"
                )
            else:
...
async def cancel_reservation_and_root(
    ...
    This handler mirrors `cancel_reservation_and_go_back` but forces a
    return to the main client menu after deleting the booking.
    """
    ...
    user = await UserRepo.get_or_create(
        user_tg_id, name=cb.from_user.full_name if cb.from_user else str(user_tg_id)
    )
...
async def select_service(cb: CallbackQuery, callback_data, state: FSMContext, locale: str) -> None:
    ...
    if user_id is None:
        return
    service_raw = str(getattr(callback_data, "service_id", "") or "")
    if "+" in service_raw:
        service_ids = [s for s in service_raw.split("+") if s]
    else:
        service_ids = [service_raw] if service_raw else []
    service_id = "+".join(service_ids) if service_ids else ""
    ...
    try:
        ...
        mids = []
    logger.debug(
        "select_service: service_id=%s -> masters_count=%d, masters=%s",
        service_id,
        len(masters_list),
        mids,
    )
...
async def show_masters_catalog(
    ...
    try:
        ...
        logger.exception("show_masters_catalog: failed to fetch masters page (page=%s)", page)
        raise
    ...
    lang = locale
    base_title = (
        t("masters_button", lang)
        if t("masters_button", lang) != "masters_button"
        else "Наші майстри"
    )
...
async def show_services_for_master(
    cb: CallbackQuery, callback_data: HasMasterId, state: FSMContext, locale: str
) -> None:
    """Показывает услуги, доступные у выбранного мастера, и включает форс-мастера для последующего шага."""
    # Normalize incoming identifier to canonical surrogate id
    try:
        resolved_master = await MasterRepo.resolve_master_id(int(callback_data.master_id))
        ...
        rows = await MasterRepo.get_services_with_durations_for_master(resolved_master)
    except Exception:
        logger.exception(
            "show_services_for_master: failed to resolve master or fetch services (master_id=%s)",
            getattr(callback_data, "master_id", None),
        )
        raise
    ...
    if rows:
        ...
        # Fallback: read duration from `Service` rows if repo rows were empty
        try:
            ...
            async with get_session() as session:
                svc_rows = (
                    await session.execute(
                        select(Service.id, Service.duration_minutes).where(
                            Service.id.in_(list(services.keys()))
                        )
                    )
                ).all()
                durations_map = {str(r[0]): int(r[1] or 0) for r in svc_rows}
        except Exception:
            durations_map = {}
...
async def select_master(
    ...
    Args:
        ...
        state: Контекст FSM для сохранения состояния.
    """
    # `user_id` and `master_id` are injected by the decorator.
    if user_id is None:
        user_id = _get_cb_user_id(cb)
    if user_id is None:
        return
    if master_id is None:
        try:
            master_id = await MasterRepo.resolve_master_id(
                int(getattr(callback_data, "master_id", 0))
            )
        except Exception:
            master_id = None
    if not master_id:
    ...
    logger.info(
        "Выбор мастера для пользователя %s, master_id=%s, service_id=%s",
        user_id,
        master_id,
        service_id,
    )
    ...
    try:
        ...
        from bot.app.services.shared_services import local_now

        now = local_now()
        year, month = now.year, now.month
        available_days = await get_available_days_for_month(
            master_id, year, month, service_duration_min=duration
        )
        sched = await MasterRepo.get_schedule(master_id)
        allowed_weekdays = (
            sorted([int(k) for k, v in (sched or {}).items() if isinstance(v, list) and v])
            if sched
            else []
        )
    ...
    try:
        ...
        from bot.app.services.shared_services import local_now as _local_now
...
async def navigate_calendar(
    ...
    """
    # `user_id` and `master_id` injected by decorator; fall back when missing
    if user_id is None:
        user_id = _get_cb_user_id(cb)
    if user_id is None:
        return
    logger.info(
        "Навигация по календарю для пользователя %s, year=%s, month=%s",
        user_id,
        callback_data.year,
        callback_data.month,
    )
    ...
    try:
        if master_id is None:
            resolved_master = await MasterRepo.resolve_master_id(
                int(getattr(callback_data, "master_id", 0))
            )
        else:
            resolved_master = master_id
        if not resolved_master:
            if cb.message:
                await safe_edit(cb.message, text=t("error_retry", locale))
            else:
        ...
        if service_ids:
            try:
                ...
                totals = await get_services_duration_and_price(
                    service_ids, online_payment=False, master_id=int(resolved_master)
                )
                ...
                if "+" in sid:
                    try:
                        ...
                        logger.exception(
                            "navigate_calendar: failed to persist multi_duration_min into state"
                        )
            except Exception:
        ...
        available_days = await get_available_days_for_month(
            resolved_master, callback_data.year, callback_data.month, sd
        )
        sched = await MasterRepo.get_schedule(resolved_master)
        allowed_weekdays = (
            sorted([int(k) for k, v in (sched or {}).items() if isinstance(v, list) and v])
            if sched
            else []
...
async def select_date(
    ...
    try:
        # master_id may be injected by decorator; resolve when missing
        if master_id is None:
            resolved_master = await MasterRepo.resolve_master_id(
                int(getattr(callback_data, "master_id", 0))
            )
    ...
    # If we're in a reschedule flow, exclude the booking being rescheduled
    # from the busy set so its current interval doesn't hide valid target slots.
    try:
        ...
        if is_reschedule:
            ...
            try:
                booking_id_value = int(raw_booking_id) if raw_booking_id is not None else None
            except Exception:
                booking_id_value = None
    except Exception:
        is_reschedule = False
    ...
    try:
        ...
        if slots:
            ...
            try:
                sample = [
                    s.strftime("%H%M") if hasattr(s, "strftime") else str(s) for s in slots[:8]
                ]
            except Exception:
    ...
    # Default to compact +/- picker: determine initial hour/minute from slots
    ih = 0
    im = 0
    try:
        if slots:
    ...
    kb = await get_compact_time_picker_kb(
        hour=ih,
        minute=im,
    ...
    if cb.message:
        # Include the selected date in the picker prompt
        try:
            date_label = format_date(datetime.fromisoformat(selected_date), "%d.%m.%Y")
        except Exception:
            date_label = selected_date or ""
        try:
            choose_time_text = t("choose_time", lang)
        except Exception:
            choose_time_text = "Choose hour on"
        prompt = f"{choose_time_text} {date_label} — {ih:02d}:{im:02d}"
        await nav_push(state, prompt, kb)
        await safe_edit(cb.message, prompt, reply_markup=kb)
    await cb.answer()
...
async def hours_view_handler(
    ...
    if not slots:
        try:
            from bot.app.services.client_services import (
                get_available_time_slots_for_services,
                get_services_duration_and_price,
            )
            ...
            if service_ids:
                try:
                    totals = await get_services_duration_and_price(
                        service_ids, online_payment=False, master_id=master_id
                    )
                    duration = int(totals.get("total_minutes") or slot_default)
                except Exception:
                    duration = slot_default
                ...
                duration = slot_default
            base_dt = datetime.fromisoformat(selected_date)
            # Exclude current booking during reschedule so availability is recomputed
            try:
                cur_state = await state.get_state()
                is_reschedule = bool(cur_state and "reschedule_select_date" in str(cur_state))
                booking_id_value = None
                if is_reschedule:
                    data = await state.get_data()
                    try:
                        booking_id_value = int((data or {}).get("cres_booking_id") or 0)
                    except Exception:
                        booking_id_value = None
            except Exception:
                booking_id_value = None
            slots = await get_available_time_slots_for_services(
                base_dt, master_id, [duration], exclude_booking_id=booking_id_value
            )
        except Exception:
            slots = []
    ...
    if not slots:
        if cb.message:
            await safe_edit(
                cb.message, t("no_time_for_date", locale), reply_markup=get_back_button()
            )
        await cb.answer()
        return
    ...
    # Preserve reschedule context when present so compact picker submits correctly
    cur_state = await state.get_state()
    is_reschedule = bool(cur_state and "reschedule" in str(cur_state))
    booking_id_value = None
    if is_reschedule:
        data = await state.get_data()
        try:
    ...
    if cb.message:
        try:
            ...
            date_label = selected_date or ""
        try:
            choose_time_text = t("choose_time", locale)
        except Exception:
            choose_time_text = "Choose hour on"
        prompt = f"{choose_time_text} {date_label}"
        await nav_push(state, prompt, kb)
        await safe_edit(cb.message, prompt, reply_markup=kb)
    # Compact picker is effectively a minute-level chooser
    await state.set_state(BookingStates.choosing_minute)
    await cb.answer()
...
async def hour_chosen_handler(
    ...
    try:
        ...
        slots = []
    logger.info(
        "hour_chosen_handler: cached slots type=%s count=%d",
        type(slots).__name__,
    ...
    if not slots:
        try:
            ...
            if service_ids:
                ...
                duration = slot_default
            base_dt = datetime.fromisoformat(selected_date)
            # When coming from reschedule flow prefer excluding the booking being moved
            try:
                cur_state = await state.get_state()
                is_reschedule = bool(cur_state and "reschedule_select_date" in str(cur_state))
                booking_id_value = None
    ...
    if not minutes or len(minutes) <= 1:
        ...
        logger.info(
            "hour_chosen_handler: fallback to tick-grid tick=%d minutes_count=%d (filtered)",
            tick,
            len(minutes),
        )
    ...
    if cb.message:
        try:
            # Show a simple date-focused header when the user is choosing minutes
            # (avoid including the hour/time in the header so the prompt reads
            # like "Оберіть годину на 2025-12-15").
            date_label = format_date(datetime.fromisoformat(selected_date), "%d.%m.%Y")
        except Exception:
            date_label = selected_date or ""
        header = f"{t('choose_time', locale)} {date_label}"
...
@client_router.callback_query(TimeAdjustCB.filter())
@require_ids(require_user=True, require_master=True)
async def compact_time_adjust_handler(
    ...
    """Handle +/- interactions for compact time picker.

    Recomputes hour/minute based on op and edits the message with a rebuilt keyboard.
    """
    if user_id is None:
        user_id = _get_cb_user_id(cb)
    if user_id is None:
    ...
    try:
        op = getattr(callback_data, "op", "noop") or "noop"
        hour = int(getattr(callback_data, "hour", 0) or 0)
        minute = int(getattr(callback_data, "minute", 0) or 0)
        service_id = str(getattr(callback_data, "service_id", "") or "")
        date = str(getattr(callback_data, "date", "") or "")
    except Exception:
        await cb.answer()
        return

    # ensure master_id available (may be injected by decorator)
    if master_id is None:
        try:
            master_id = int(getattr(callback_data, "master_id", 0) or 0)
        except Exception:
    ...
    # Determine minute step from settings if available
    try:
        tick = await SettingsRepo.get_slot_tick_minutes()
        tick = int(tick or 15)
    except Exception:
    ...
    # Prefer stepping through available slots cached in FSM for this date.
    try:
        data = await state.get_data()
        slots_map = (data or {}).get("slots_for_date") if isinstance(data, dict) else None
        cached_for_date = None
        if isinstance(slots_map, dict):
            cached_for_date = slots_map.get(date) or None
        if cached_for_date and isinstance(cached_for_date, list) and len(cached_for_date) > 0:
            # cached slot strings like '0900', '0945'
            slots_int = []
            for s in cached_for_date:
                try:
                    slots_int.append(int(s))
                except Exception:
                    continue
            slots_int = sorted(set(slots_int))
            # build hour->minutes map
            hours_map: dict[int, list[int]] = {}
            for v in slots_int:
                h = v // 100
                m = v % 100
            ...
            hours_list = sorted(hours_map.keys())

            cur_h = int(hour)
            cur_m = int(minute)

            if op == "hour_inc":
                # Move to the next available hour only; do not automatically change minute
                nxt = None
                for hh in hours_list:
                    if hh > cur_h:
                        ...
                        break
                if nxt is not None:
                    # If the same minute exists in the target hour, keep it; otherwise
                    # snap to the closest available minute in that hour so the selection
                    # remains valid (prevents choosing unavailable starts).
                    if cur_m in hours_map.get(nxt, []):
                        hour = nxt
                    else:
                        candidates = hours_map.get(nxt, [])
                        if candidates:
                            # pick nearest minute to current minute
                            minute = min(candidates, key=lambda x: abs(x - cur_m))
                            hour = nxt
                # else: keep current hour/minute (ignore)
            elif op == "hour_dec":
                prev = None
                for hh in reversed(hours_list):
                    if hh < cur_h:
                        prev = hh
                        break
                if prev is not None:
                    if cur_m in hours_map.get(prev, []):
                        hour = prev
                    else:
                        candidates = hours_map.get(prev, [])
                        if candidates:
                            minute = min(candidates, key=lambda x: abs(x - cur_m))
                            hour = prev
                # else: keep current hour/minute (ignore)
            elif op == "min_inc":
                # Move forward by tick across available slots (may cross to next hour).
                # Build flattened minutes-since-midnight list and find first slot >= target.
                slots_minutes = []
                for v in slots_int:
                    try:
                        h = v // 100
                        m = v % 100
                        slots_minutes.append(h * 60 + m)
                    except Exception:
                        continue
                slots_minutes = sorted(set(slots_minutes))
                target = cur_h * 60 + cur_m + tick
                nxt_total = None
                for tot in slots_minutes:
                    if tot >= target:
                        nxt_total = tot
                        break
                if nxt_total is not None:
                    hour = nxt_total // 60
                    minute = nxt_total % 60
                # else: no later slot on that day -> ignore (no wrap)
            elif op == "min_dec":
                # Move backward by tick across available slots (may cross to previous hour).
                slots_minutes = []
                for v in slots_int:
                    try:
                        h = v // 100
                ...
                target = cur_h * 60 + cur_m - tick
                prev_total = None
                for tot in reversed(slots_minutes):
                    if tot <= target:
                        prev_total = tot
                        break
                if prev_total is not None:
                    hour = prev_total // 60
                    minute = prev_total % 60
                # else: no earlier slot on that day -> ignore
            # ensure ints
            hour = int(hour)
            minute = int(minute)
        else:
            # Fallback to tick-based adjustment when no cached slots exist.
            # Use non-wrapping behavior: clamp hours to [0,23], minutes to [0,59].
            if op == "hour_inc":
    ...
    try:
        # Preserve reschedule context when adjusting compact picker so Submit uses RescheduleCB
        cur_state = await state.get_state()
        is_reschedule = bool(cur_state and "reschedule" in str(cur_state))
        booking_id_value = None
        if is_reschedule:
    ...
    if cb.message:
        try:
            try:
                date_label = format_date(datetime.fromisoformat(date), "%d.%m.%Y")
            except Exception:
                date_label = date or ""
            header = f"{t('choose_time', locale)} {date_label} — {hour:02d}:{minute:02d}"
        except Exception:
            header = f"{date or ''} — {hour:02d}:{minute:02d}"
        try:
            await safe_edit(cb.message, header, reply_markup=kb)
        except Exception:
            # Fallback: answer and do nothing
            logger.exception("compact_time_adjust_handler: failed to edit message")
    await cb.answer()
...
async def _fetch_services_and_totals(state: FSMContext | None, selected: set[str] | None = None):
    ...
    # Resolve global currency for totals
    try:
        from bot.app.services.admin_services import SettingsRepo
    ...
    try:
        ...
        if selected:
            totals = await get_services_duration_and_price(
                list(selected), master_id=int(forced_master) if forced_master is not None else None
            )
    except Exception:
        totals = {"total_minutes": 0, "total_price_cents": 0, "currency": global_currency}
...
async def services_multi_entry(
    ...
    if selected:
        ...
        prompt = (
            t("multi_selected_summary", lang)
            or "Selected: {count} services ({hours} h, ~{price} {currency})"
        ).format(
            count=len(selected),
            hours=f"{hours:.1f}".rstrip("0").rstrip("."),
            price=price,
            currency=currency,
        )
    else:
...
async def svc_toggle(
    cb: CallbackQuery, callback_data: HasServiceId, state: FSMContext, locale: str
) -> None:
...
async def master_multi(
    cb: CallbackQuery, callback_data: HasMasterId, state: FSMContext, locale: str
) -> None:
    """Show master profile first for multi-service flow, then let user proceed to calendar."""
    # Resolve to canonical surrogate id
    master_id = await MasterRepo.resolve_master_id(int(callback_data.master_id))
    if not master_id:
    ...
    try:
        ...
        totals = await get_services_duration_and_price(
            selected, online_payment=False, master_id=master_id
        )
...
async def select_time_and_create_booking(
    ...
    service_id = callback_data.service_id
    # Normalize master id to surrogate id before attempting booking
    master_id = await MasterRepo.resolve_master_id(int(callback_data.master_id))
    if not master_id:
        await cb.answer(t("invalid_data", locale), show_alert=True)
        return
    ...
    if not result.get("ok"):
        ...
        if code == "client_already_has_booking_at_this_time":
            await cb.answer(
                t("you_already_have_booking_at_this_time", lang)
                or "У вас уже есть запись на это время",
                show_alert=True,
            )
            ...
            await cb.answer(
                t("booking_failed", lang) or "Не удалось создать запись. Попробуйте снова.",
                show_alert=True,
            )
        return
    ...
    header, kb = await get_payment_keyboard(
        booking, service_name, master_name, cb.from_user.id, date=formatted_date
    )
    if cb.message:
...
async def pay_cash_prepare(
    ...
    # Append start time and duration lines to match payment preview formatting
    header = details
    try:
        ...
        if extra_lines:
            ...
            header = (
                f"<b>{t('pay_cash_confirm_title', lang)}</b>\n\n"
                + header
                + "\n"
                + "\n".join(extra_lines)
            )
        else:
            header = f"<b>{t('pay_cash_confirm_title', lang)}</b>\n\n" + header + "\n"
    except Exception:
        header = f"{details}\n"
    ...
    kb = get_simple_kb(
        [
            ...
            (
                t("back_to_payment_methods", lang),
                pack_cb(PayCB, action="back_methods", booking_id=int(booking_id)),
            ),
        ],
        cols=1,
    )
...
async def handle_rating(cb: CallbackQuery, callback_data, state: FSMContext, locale: str) -> None:
    ...
    try:
        if cb.message:
            ...
            prompt = t("rate_prompt_title", lang)
            if prompt and prompt in txt:
                txt = txt.replace(prompt, rating_line)
            else:
...
async def pay_cash(
    cb: CallbackQuery, callback_data: HasBookingId, state: FSMContext, locale: str
) -> None:
    """
    ...
    logger.info(
        "analytics.cash_confirm_click user_id=%s booking_id=%s", cb.from_user.id, booking_id
    )
...
# Specialized back_to_masters removed; use global navigation.
...
async def my_bookings(
    ...
    await state.update_data(
        my_bookings_filter=new_filter, my_bookings_page=page, current_screen="my_bookings"
    )
    ...
    try:
        ...
        kb = await build_my_bookings_keyboard(
            ...
            cancelled_count=meta.get("cancelled_count", 0) if meta else 0,
            noshow_count=meta.get("noshow_count", 0) if meta else 0,
            total_pages=meta.get("total_pages") if meta else 1,
        )
    ...
    await cb.answer()
    logger.info(
        "Список бронирований отображен для пользователя %s (количество=%d)", user_id, len(rows)
    )
...
async def cancel_booking_confirm(cb: CallbackQuery, callback_data, locale: str) -> None:
    ...
    lang = locale
    lock_m = await _int_setting(SettingsRepo.get_client_cancel_lock_minutes, 180)
    b = await BookingRepo.get(booking_id)
    # If booking not found, inform user
    if not b:
        await cb.answer(t("booking_not_found", lang), show_alert=True)
        return
    ...
    try:
        ...
        status_val = status_attr
    if str(status_val).lower() in terminal_statuses:
        await cb.answer(t("booking_not_active", lang), show_alert=True)
        return

    if (b.starts_at - utc_now()).total_seconds() < lock_m * 60:
        # Use compact label for user-facing alert
        if lock_m >= 120 and lock_m % 60 == 0:
            lock_label = f"{lock_m // 60} {t('hours_short', lang) or 'h'}"
        else:
            lock_label = f"{lock_m} {t('minutes_short', lang) or 'min'}"
        logger.info(
            "analytics.cancel_too_close preconfirm user_id=%s booking_id=%s minutes=%s",
            cb.from_user.id,
            booking_id,
            lock_m,
        )
        await cb.answer(t("cancel_too_close", lang).format(minutes=lock_label), show_alert=True)
        return
    from aiogram.utils.keyboard import InlineKeyboardBuilder
...
async def master_profile_handler(
    cb: CallbackQuery, callback_data: Any, state: FSMContext, locale: str
) -> None:
    """Обрабатывает переход в профиль мастера из меню выбора."""
    # Resolve to canonical surrogate id before showing profile
    try:
        master_id = await MasterRepo.resolve_master_id(int(callback_data.master_id))
        if not master_id:
            await cb.answer(t("master_not_found", locale), show_alert=True)
            return
    except Exception:
        await cb.answer(t("error_retry", locale), show_alert=True)
        return
    service_id = callback_data.service_id
    await show_master_profile(cb, master_id, service_id, state, locale)
...
async def show_master_profile(
    cb: CallbackQuery, master_id: int, service_id: str, state: FSMContext, locale: str
) -> None:
    ...
    # Fetch profile data directly from repository
    data = await MasterRepo.get_master_profile_data(master_id) or {}
    ...
    from bot.app.services.master_services import format_master_profile_text as _fmt
...
async def client_reschedule_start(
    ...
    try:
        ...
        async with get_session() as session:
            bi_rows = (
                await session.execute(
                    select(BookingItem.service_id).where(BookingItem.booking_id == booking_id)
                )
            ).all()
        if bi_rows:
    ...
    await state.update_data(
        cres_booking_id=booking_id, service_id=service_id_str, master_id=master_id
    )
...
async def client_reschedule_time(
    ...
    now_utc = utc_now()
    if (local_dt.astimezone(UTC) - now_utc).total_seconds() < lock_m * 60:
    ...
    if cb.message:
        await safe_edit(
            cb.message,
            f"{t('reschedule_confirm_time_prefix', lang)} {date_str} {hh}:{mm}?",
...
async def client_reschedule_confirm(
    cb: CallbackQuery, callback_data: Any, state: FSMContext, locale: str
) -> None:
    ...
    user = await UserRepo.get_or_create(
        cb.from_user.id, name=cb.from_user.full_name or str(cb.from_user.id)
    )
    b = await BookingRepo.ensure_owner(int(user.id), booking_id)
    if not b:
        await cb.answer(t("booking_not_found", lang), show_alert=True)
...
async def pay_online(cb: CallbackQuery, callback_data, locale: str) -> None:
    ...
    master_name = await MasterRepo.get_master_name(int(getattr(booking, "master_id", 0))) or t(
        "master_label", locale
    )

    # Проверка цены (fallback to Service.price_cents if snapshots are missing)
    price_cents = getattr(booking, "final_price_cents", None) or getattr(
        booking, "original_price_cents", None
    )
    ...
    currency = normalize_currency(currency) or _default_currency()

    # If using a Telegram test provider token, force USD (Telegram test providers are USD-only)
    provider_token = (await get_telegram_provider_token()) or ""
    if ":TEST:" in provider_token.upper() and currency != "USD":
        currency = "USD"

    # Telegram requires minor units to match currency exponent
    TELEGRAM_EXPONENTS = {
        "BHD": 3,
        "JOD": 3,
        "KWD": 3,
    ...
    try:
        ...
        if exp == 2:
            ...
            factor = Decimal(10) ** Decimal(exp - 2)
            amount_minor = int((price_minor * factor).to_integral_value(rounding=ROUND_HALF_UP))
        else:  # exp < 2 (e.g., 0)
    ...
    if cb.message:
        await cb.message.answer_invoice(
            ...
            description=f"{service_name} у {master_name} {format_date(getattr(booking, 'starts_at', None), '%d.%m %H:%M')}",
...
async def pay_online_prepare(
    ...
    kb = get_simple_kb(
        [
            (t("pay_now", lang), pack_cb(PayCB, action="online", booking_id=booking_id)),
            (
                t("back_to_payment_methods", lang),
                pack_cb(PayCB, action="back_methods", booking_id=booking_id),
            ),
        ],
        cols=1,
    )
    ...
    if discount_pct and savings_cents > 0:
        ...
        disc_label = (
            t("online_discount_label_plain", lang)
            or t("online_discount_label", lang)
            or "Online discount"
        )
        try:
            savings_text = format_money_cents(
                savings_cents, getattr(bd_for_display, "currency", None)
            )
        except Exception:
    ...
    start_time_str = None
    try:
        if getattr(bd, "starts_at", None):
            from bot.app.services.shared_services import format_date

            start_time_str = format_date(bd.starts_at, fmt="%H:%M")
    except Exception:
        try:
            start_time_str = bd.starts_at.strftime("%H:%M")
        except Exception:
            start_time_str = None
...
async def pay_back_methods(
    ...
    header, kb = await get_payment_keyboard(
        booking_payload,
        bd.service_name or t("service_label", lang),
        bd.master_name,
        cb.from_user.id,
        date=bd.date_str,
    )
...
async def cancel_booking(cb: CallbackQuery, callback_data, state: FSMContext, locale: str) -> None:
    ...
    bot = cb.bot or (cb.message.bot if getattr(cb, "message", None) is not None else None)
...
async def master_info_handler(callback: CallbackQuery, callback_data: Any, locale: str) -> None:
    ...
    # Resolve to canonical surrogate id before retrieving profile data
    try:
        master_id = await MasterRepo.resolve_master_id(int(callback_data.master_id))
        if not master_id:
        ...
        await callback.answer(t("error_retry", lang), show_alert=True)
        return
    ...
    try:
        ...
        logger.info(
            "Информация о мастере %s показана пользователю %s", master_id, callback.from_user.id
        )
        return
    except (ImportError, AttributeError, ValueError):
        # Fall back to older inline logic only if service/formatter fail
        logger.exception(
            "Ошибка в master_services.format_master_profile_data или formatter, falling back to inline",
            exc_info=True,
        )
    ...
    try:
        ...
        await callback.answer(text, show_alert=True)
        logger.info(
            "Информация о мастере %s показана пользователю %s (fallback)",
            master_id,
